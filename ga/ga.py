...
class GeneticAlgorithm:
    def __init__(
        self, target: float, initial_pool: list = [], dao=SolutionAttemptsDAO()
    ):
        self.target = target
        self.dao = dao
        if initial_pool != []:
    ...
    def evolve(self):
        while True:
            ...
            self.pool.sort(
                key=lambda i: max([v for k, v in i.fitness.items()]), reverse=True
            )
            # select two fittest
            parents = self.pool[:2]
            ...
            skips = (
                json.dumps(offspring.crypto.skips)
                if offspring.crypto.skips is not None
                else None
            )
            excludes = (
                json.dumps(offspring.crypto.excludes)
                if offspring.crypto.excludes is not None
                else None
            )
            ...
            max_confidence_lang = next(
                k for k, v in offspring.fitness.items() if v == max_confidence
            )
            self.dao.insert_solution_attempt(
                section_name[4:],
                offspring.retrieval.nums,
                scheme_name,
                offspring.crypto.key,
