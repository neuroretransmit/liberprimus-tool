    ...
    with open("./data/transcription.txt") as lp:
        ...
        for num in nums:
            if num < 0 or num >= len(ends):
                raise ValueError(
                    f"{ENTITY_LOOKUP[delimiter]} index {num} is out of range."
                )
            else:
...
def print_section_context(
    page_num, segment_num, paragraph_num, sentence_num, word_num, text
):
    print(
        f"=== PAGE {page_num}:SEGMENT {segment_num}:PARAGRAPH {paragraph_num};SENTENCE {sentence_num};WORD {word_num} CONTEXT ==="
    )
    print("Imagery or context information goes here")
    print("===============================")
    print(text)
