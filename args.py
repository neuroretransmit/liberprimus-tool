...
def get_transcription_validations():
    with open("./data/transcription.txt", "r", encoding="utf-8") as lp:
        ...
        segments_per_page = find_occurences(text, range(len(pages)), "$")
        paragraphs = list(find(text, "&"))
        paragraphs_per_page = find_occurences(text, range(len(pages)), "&")
        paragraphs_per_segment = find_occurences(text, range(len(segments)), "&")
        clauses = list(find(text, "."))
        clauses_per_page = find_occurences(text, range(len(pages)), ".")
        clauses_per_segment = find_occurences(text, range(len(segments)), ".")
        clauses_per_paragraph = find_occurences(text, range(len(paragraphs)), ".")
        lines = list(find(text, "/"))
        lines_per_page = find_occurences(text, range(len(pages)), "/")
        lines_per_paragraph = find_occurences(text, range(len(paragraphs)), "/")
        lines_per_clause = find_occurences(text, range(len(clauses)), "/")
        lines_per_segment = find_occurences(text, range(len(segments)), "/")
        words = list(find(text, "-"))
        words_per_page = find_occurences(text, range(len(pages)), "-")
        words_per_paragraph = find_occurences(text, range(len(paragraphs)), "-")
        words_per_clause = find_occurences(text, range(len(clauses)), "-")
        words_per_segment = find_occurences(text, range(len(segments)), "-")
        words_per_line = find_occurences(text, range(len(lines)), "-")
        return {
            "pages": {
                "num": len(pages),
            },
...
def parse_args():
    ...
    if args.pages:
        ...
        if (
            len(pages) > 0
            and len(pages - set(range(0, validations["pages"]["num"]))) > 0
        ):
            raise ValueError(
                f"Pages must be in range of 0-{validations['pages']['num']}"
            )
        target["pages"] = pages
    if args.lines:
        ...
        if (
            len(lines) > 0
            and len(lines - set(range(0, validations["lines"]["num"]))) > 0
        ):
            raise ValueError(
                f"Lines must be in range of 0-{validations['lines']['num']}"
            )
        target["lines"] = lines
    if args.segments:
    ...
    if args.paragraphs:
        ...
        if (
            len(paragraphs) > 0
            and len(paragraphs - set(range(0, validations["paragraphs"]["num"]))) > 0
        ):
            raise ValueError(
                f"Paragraphs must be in range of 0-{validations['paragraphs']['num']}"
            )
    ...
    if args.clauses:
        ...
        if (
            len(clauses) > 0
            and len(clauses - set(range(0, validations["clauses"]["num"]))) > 0
        ):
            raise ValueError(
                f"Clauses must be in range of 0-{validations['clauses']['num']}"
            )
    ...
    if args.words:
        ...
        if (
            len(words) > 0
            and len(words - set(range(0, validations["words"]["num"]))) > 0
        ):
            raise ValueError(
                f"Words must be in range of 0-{validations['words']['num']}"
            )
        target["words"] = words
    if args.runes:
